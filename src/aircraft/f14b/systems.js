...
const INS_TIME = { now:20, coarse:100, min:250, fine:480, cva:540 };   // shore 8 min, boat 9
...
export function onChange(sim, id, to) {
    ...
    switch(id){
      ...
      case 'radAltKnob':
        if(to==='on'){ S.radalt.t=0; S.radalt.value=5000; sim.emit('Radar altimeter BIT running — needle to max, then 100 ft.','radio'); }
...
export function tick(sim, dt) {
    ...
    // hydraulics
    const spun = Math.max(S.eng.L.n2, S.eng.R.n2) > 55;
    const fltT = spun ? 3000 : (S.sw.emergFltHyd==='auto' && spun ? 2200 : 0);
    const cmbT = (spun && S.sw.hydTransfer==='norm') ? 3000 : 0;
    ...
    const R=S.rio;
    const wcsOk = S.power && S.sw.wcsMode!=='off' && S.sw.liquidCool==='fwd' &&
                  S.sw.airSource==='both' && S.eng.L.n2>55 && S.eng.R.n2>55;
    if(wcsOk && !R.wcsUp){
      R.wcsT += dt;
      if(R.wcsT >= 40){ R.wcsUp=true; sim.emit('WCS up — TID and DDD online.','good'); }
    ...
    if(S.sw.radAltKnob!=='on'){ S.radalt.value=0; }
      ...
      S.radalt.t += dt;
      // needle slams to max as it powers up, then parks at 100 ft while it tests
      S.radalt.value = S.radalt.t < 3 ? 5000 : 100 + Math.sin(S.t*1.7)*4;
